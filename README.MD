...
用 Go 编写的 MCP 搜索服务，内置百度网页搜索、Bing、DuckDuckGo 等通用搜索引擎和 6 个学术搜索引擎。可作为 MCP 工具接入 Claude Code、Qwen Code、Cursor 等客户端，也可作为 Go 模块嵌入自有服务。
...
**面向 LLM 的完整工具链** — 4 个工具覆盖联网工作流：`smartsearch` 搜索 → `cleanfetch` 抓取原文 → `pdf_parser` 解析 PDF → LLM 摘要精炼，结果互相衔接，一次配置全链路可用。

**场景化优化** — 针对真实使用场景：学术搜索（6 引擎 + 引用 / 期刊 / PDF 评分）、国内网络（直连 + 系统代理自动检测）、扫描件 PDF（MinerU OCR 回退）、时效性查询（`time_range`）。
...
#    首次 start 会在可执行文件目录自动生成一份可编辑的预设 config.yaml
...
| 多引擎融合 | 6 种搜索模式、7 个通用引擎 + 6 个学术引擎，主引擎失败自动回退 |
| 相关性评分 | RRF 融合排名 + 词汇对齐 / 域名品质 / 共识 / 权威 / 时效加分，低分结果自动裁剪；MMR 打散同主题高相似结果（转载 / 镜像站） |
| 学术搜索 | 6 大学术引擎并行，按引用数 / 期刊权威 / PDF 可用性 / 新鲜度评分 |
...
| `apipool` | API Key 池轮转：每次只调一个供应商，失败自动切换 | 各 Key 可选 |
| `tavily` | Tavily Search API | `TAVILY_SK` |
| `exa` | Exa Web Search API | `EXA_API_KEY` |
| `hybrid` | 全引擎混合（百度 + Tavily + Exa + Bing + DuckDuckGo 等） | 各 Key 可选 |
